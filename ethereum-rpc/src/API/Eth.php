...
class Eth
{
    ...
    {
        $request = $this->client->jsonRPC("eth_gasPrice");
        $gasPrice = $request->get("result");
        if (!is_string($gasPrice)) {
            throw GethException::unexpectedResultType("eth_gasPrice", "string", gettype($gasPrice));
        }
        $gasPrice = strval(BcMath::HexDec($gasPrice));
        return bcdiv($gasPrice, bcpow("10", "9", 0), 2);
        
    }

    public function getSyncing()
    {
        $request = $this->client->jsonRPC("eth_syncing");
        $syncing = $request->get("result");
        return $syncing;
    }
     ...
     * @throws \HttpClient\Exception\HttpClientException
     */
    public function sendTransaction($data): string
    {
        $value = "0x" . BcMath::DecHex(bcmul($data['amount'], bcpow("10", "18"), 0));
        $gas = "0x" . BcMath::DecHex($data['gas']);
        $gasPrice = "0x" . BcMath::DecHex(bcmul($data['gasPrice'], bcpow("10", "12"), 0));
        $transaction = [
            "from" => $data['from'],
            "to" => $data['to'],
            'gas'=>$gas,
